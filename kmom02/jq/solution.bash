...
# 3. Filen innehåller en länk till nästa resultat i flödet, en så kallad
#    pagination. Visa länken till nästa länk (next_url) (c.txt).
...
# 7. Den första bilden är tagen på en viss plats (location) som har ett visst
#    namn (name), vilket (g.txt)?
jq '.data[0].location.name' tag-dbwebb.json | tee g.txt

# 8. Den första bilden har en kommentar (comments), visa texten (text) för
#    kommentaren (h.txt).
jq '.data[0].comments.data[0].text' tag-dbwebb.json | tee h.txt

# 9. Den första bilden har flera likes. Visa namnet på den andra personen som
#    gjorde like på bilden (i.txt).
...
# 11. Användaren alkifaey har gjort en kommentar. Använd användarnamnet och
#     visa själva kommentaren (text) (k.txt).
jq '.data[].comments.data[] | select(.from.username == "alkifaey").text' tag-dbwebb.json | tee k.txt

# 12. Lista länkarna (link) till bilderna som användaren ‘tobhed’ har
#     kommenterat (l.txt).
...
jq '.data[] | select(.likes.count > 5 and .comments.count >= 1).link' tag-dbwebb.json | tee n.txt

# 15. Hitta användarnamnen (username) på de användare som likeat bilden som
